...
@Tag(
    name = "CRUD REST APIs for Loans in NexBank",
...
public class LoanController {
    ...
    @ApiResponses(
        value = {
            @ApiResponse(responseCode = "201", description = "HTTP Status CREATED"), @ApiResponse(
    ...
    @PutMapping
    public ResponseEntity<RestResponse> updateLoanDetails(
        @Valid
        @RequestBody
        LoanDTO loansDto) {
        ...
        if (isUpdated) {
            ...
            return ResponseEntity.status(HttpStatus.EXPECTATION_FAILED)
                .body(RestResponse.error(LoanConstants.STATUS_EXPECTATION_FAILED, LoanConstants.MESSAGE_UPDATE_FAILED));
        }
    }
